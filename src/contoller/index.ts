...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const token = jwt.sign(
      { id: chackUser.id, email: chackUser.email, role: chackUser.role },
      "SECRET_KEY",
      { expiresIn: "1h" }
    );
...
export const getContestData = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        dsaProblems: dsaProblems.map((dsaProblem: any) => ({
          id: dsaProblem.id,
...
export const submitMCQAnswers = async (req: Request, res: Response) => {
  try {
    ...
    const chackcontectActive = await PrismaClient.contests.findUnique({
      where: { id: contestId },
    });
    if (!chackcontectActive) {
      return res.status(404).json({
        success: false,
        data: null,
        error: "CONTEST_NOT_FOUND",
    ...
    if (
      currentTime < chackcontectActive.start_time ||
      currentTime > chackcontectActive.end_time
    ) {
    ...
    const mcqQuestion = await PrismaClient.mcq_questions.findUnique({
      where: { id: questionId, contest_id: contestId },
    });
    if (!mcqQuestion) {
      return res.status(404).json({
        success: false,
    ...
    const allradySubmitted = await PrismaClient.mcq_submissions.findFirst({
      where: {
        user_id: user_id,
